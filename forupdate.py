...
class CoffeeShopApp:
    ...
    # ------------- Login Screen ------------- #
    def prevent_close():
     messagebox.showwarning("غير مسموح", "يجب تسجيل الدخول أولاً.")
    ...
    def create_main_screen(self):
        ...
        # Image=CTkImage(Image.open('my_logo.jpg') ,size=(200,100))
        label_title = tk.Label(self.root, text="بن البركة" ,font=("Arial", 16, "bold"), fg="brown", bg="#f0f0f0")
        label_title.pack(pady=10)

        btn_frame = tk.Frame(self.root, bg="#ffffff")
        btn_frame.pack(pady=20)   

        tk.Button(btn_frame, text="تسجيل عميل جديد", font=("Arial", 16, "bold" ), width=22, bg="#FF9817",fg="#ffffff", command=self.open_add_customer).pack(pady=5)
        tk.Button(btn_frame, text="بحث", font=("Arial", 16, "bold"), width=22, bg="#FF9817",fg="#ffffff", command=self.open_search).pack(pady=5)
        tk.Button(btn_frame, text="تسجيل موظف جديد", font=("Arial", 16, "bold"), width=22 , bg="#FF9817",fg="#ffffff", command=self.open_employee_screen).pack(pady=5)
        tk.Button(btn_frame, text="عرض العملاء", font=("Arial", 16, "bold"), width=22, bg="#FF9817",fg="#ffffff" , command=self.open_all_customers_screen).pack(pady=5)
        tk.Button(btn_frame, text="🔄 إنشاء نسخة احتياطية", font=("Arial", 16, "bold"), width=22 , bg="#FF9817",fg="#ffffff", command=self.manual_backup).pack(pady=5)
    ...
    def manual_backup(self):
        ...
        if folder:
            ...
            messagebox.showinfo("نجاح", "تم حفظ نسخة احتياطية من قاعدة البيانات.")
################################################
    ...
    def open_add_customer(self):
        ...
        add_win.geometry("1000x900+20+20")
        add_win.config(bg="#FFD65A")
        ...
        title_frame.config(bg="#FFD65A")
        title_frame.pack(fill=tk.X, padx=10, pady=5)

        title = tk.Label(title_frame, text="تسجيل عميل جديد", font=("Arial", 16, "bold"), fg="#4B352A" ,bg="#FFD65A")
        title.pack(pady=5)
        # Customer form using grid
        form_frame = tk.Frame(add_win)
        form_frame.config(bg="#FFD65A")
        form_frame.pack(fill=tk.X, padx=100, pady=5)
        lbl_customer_id = tk.Label(form_frame, text="رقم العميل", anchor="e", width=12, font=("Arial", 16 , "bold")  , fg="#4B352A" ,bg="#FFD65A")
        lbl_customer_id.grid(row=0, column=1, padx=5, pady=5)
        self.entry_customer_id = tk.Entry(form_frame, justify="right",width=50)
        self.entry_customer_id.grid(row=0, column=0, padx=5, pady=5)
        self.entry_customer_id.config(state='disabled')
        lbl_first_name = tk.Label(form_frame, text="الاسم الأول", anchor="e", width=12 ,font=("Arial", 16 , "bold")  , fg="#4B352A",bg="#FFD65A")
        lbl_first_name.grid(row=1, column=1, padx=5, pady=5)
        self.entry_first_name = tk.Entry(form_frame, justify="right" ,width=50 ,font=("Arial", 16 , "bold"))
        self.entry_first_name.grid(row=1, column=0, padx=5, pady=5)
        lbl_last_name = tk.Label(form_frame, text="الاسم الثانى", anchor="e", width=12 ,font=("Arial", 16 , "bold")  , fg="#4B352A" ,bg="#FFD65A")
        lbl_last_name.grid(row=2, column=1, padx=5, pady=5)
        self.entry_last_name = tk.Entry(form_frame, justify="right" ,width=50 ,font=("Arial", 16 , "bold"))
        self.entry_last_name.grid(row=2, column=0, padx=5, pady=5)
        lbl_phone = tk.Label(form_frame, text="رقم التليفون", anchor="e", width=12,font=("Arial", 16 , "bold")  , fg="#4B352A" ,bg="#FFD65A")
        lbl_phone.grid(row=3, column=1, padx=5, pady=5)
        self.entry_phone = tk.Entry(form_frame, justify="right" ,width=50,font=("Arial", 16 , "bold") )
        self.entry_phone.grid(row=3, column=0, padx=5, pady=5)
        # Instead of free text entry, use a Combobox for employee names
        lbl_employee = tk.Label(form_frame, text="الموظف", anchor="e", width=12 ,font=("Arial", 16 , "bold")  , fg="#4B352A" ,bg="#FFD65A")
        lbl_employee.grid(row=4, column=1, padx=5, pady=5)
        self.employee_combobox = ttk.Combobox(form_frame, justify="right" , width=49 ,font=("Arial", 16 , "bold"))
...
# Auto focus chain
        self.entry_first_name.bind("<Return>", lambda e: self.entry_last_name.focus_set())
        self.entry_last_name.bind("<Return>", lambda e: self.entry_phone.focus_set())
        self.entry_phone.bind("<Return>", lambda e: self.employee_combobox.focus_set())
        self.employee_combobox.bind("<Return>", lambda e: self.radio_sada.invoke())

        # Mix category section (now radio buttons)
        category_frame = tk.LabelFrame(add_win, text="اختر نوع التوليفة", padx=10, pady=5, font=("Arial", 10))
        category_frame.pack(pady=10)

        self.mix_category = tk.StringVar()
        self.mix_category.set("")  # No selection by default

        self.radio_sada = tk.Radiobutton(category_frame, text="سادة", variable=self.mix_category, value="سادة", anchor="w", justify="right")
        self.radio_sada.grid(row=0, column=0, padx=5)
        self.radio_mehaweg = tk.Radiobutton(category_frame, text="محوج", variable=self.mix_category, value="محوج", anchor="w", justify="right")
        self.radio_mehaweg.grid(row=0, column=1, padx=5)
        self.radio_ispresso = tk.Radiobutton(category_frame, text="اسبريسو", variable=self.mix_category, value="اسبريسو", anchor="w", justify="right")
        self.radio_ispresso.grid(row=0, column=2, padx=5)

        # Entry for mix details
        mix_input_frame = tk.Frame(add_win)
        mix_input_frame.pack(pady=5)
        tk.Label(mix_input_frame, text="تفاصيل التوليفة:", anchor="e").pack(side=tk.RIGHT, padx=5)
        #
        self.entry_mix_details = tk.Entry(mix_input_frame, justify="right", width=60)
        self.entry_mix_details.pack(side=tk.RIGHT, padx=5)
        self.entry_mix_details.bind("<Return>", lambda e: btn_add_mix.focus_set())

        btn_add_mix = tk.Button(add_win, text="أضف توليفة", width=10 ,bg="#4B352A" ,fg="white",command=self.add_mix_row)
        btn_add_mix.pack(pady=5)
        ...
        tree_frame = tk.Frame(mix_input_frame)
        ...
        btn_save = tk.Button(btn_frame, text="حفظ", width=10 ,bg="#4B352A" ,fg="white" ,command=self.save_customer)
        btn_save.pack(side=tk.RIGHT, padx=10)
        btn_cancel = tk.Button(btn_frame, text="الغاء", width=10 ,bg="#4B352A" ,fg="white", command=add_win.destroy) 
        btn_cancel.pack(side=tk.RIGHT, padx=10)
        self.mix_rows = []
        self.entry_first_name.focus_set()

    def add_mix_row(self):
            category = self.mix_category.get()
            details = self.entry_mix_details.get().strip()

            if not category:
                messagebox.showwarning("تنبيه", "يرجى اختيار نوع التوليفة")
                return
            if not details:
                messagebox.showwarning("تنبيه", "يرجى إدخال تفاصيل التوليفة")
                return
            temp_id = len(self.mix_rows) + 1 
            date_str = datetime.now().strftime("%Y-%m-%d")
                   
            row = (temp_id,date_str, category, details)
            self.tree_mixes.insert("", "end", values=row)
            self.mix_rows.append(row)
            self.entry_mix_details.delete(0, tk.END)
            self.mix_category.set("")
    ...
    def save_customer(self):
        ...
        self.mix_category.set("")
        self.entry_mix_details.delete(0, tk.END)
        for row in self.tree_mixes.get_children():
         self.tree_mixes.delete(row)
         self.mix_rows.clear()
    ...
    # ------------- Search Screen ------------- #
    def show_customer_details_from_search(self, event):
        selected = self.search_result_tree.selection()
        if not selected:
            return

        item = self.search_result_tree.item(selected[0])
        cust_id = item["values"][0]  # ID is first column

        # Fetch full customer details
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute("SELECT first_name, last_name, phone, employer FROM customers WHERE id = ?", (cust_id,))
        cust = cursor.fetchone()

        cursor.execute("SELECT date, category, details FROM mixes WHERE customer_id = ? ORDER BY date DESC", (cust_id,))
        mixes = cursor.fetchall()
        conn.close()

        # Build details window
        win = tk.Toplevel(self.root)
        win.title("تفاصيل العميل")
        win.geometry("600x400")

        full_name = f"{cust[0]} {cust[1]}"
        tk.Label(win, text=f"الاسم: {full_name}", font=("Arial", 12)).pack(pady=5)
        tk.Label(win, text=f"رقم الهاتف: {cust[2]}", font=("Arial", 12)).pack(pady=5)
        tk.Label(win, text=f"الموظف: {cust[3]}", font=("Arial", 12)).pack(pady=5)

        tk.Label(win, text="سجل التوليفات", font=("Arial", 13, "bold")).pack(pady=10)

        tree = ttk.Treeview(win, columns=("date", "category", "details"), show="headings")
        tree.heading("date", text="التاريخ")
        tree.heading("category", text="النوع")
        tree.heading("details", text="التفاصيل")
        tree.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        for mix in mixes:
            tree.insert("", "end", values=mix)


    def open_search(self):
        
        search_win = tk.Toplevel(self.root)
        search_win.title("بحث عن عميل")
        search_win.geometry("600x400")

        # Title
        tk.Label(search_win, text="بحث عن عميل", font=("Arial", 14, "bold")).pack(pady=10)

        # Search mode selection (radiobuttons)
        self.search_mode = tk.StringVar(value="name")
        filter_frame = tk.Frame(search_win)
        filter_frame.pack(pady=5)

        tk.Label(filter_frame, text="بحث حسب:", font=("Arial", 11)).pack(side=tk.LEFT, padx=10)
        tk.Radiobutton(filter_frame, text="الاسم", variable=self.search_mode, value="name").pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(filter_frame, text="رقم التليفون", variable=self.search_mode, value="phone").pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(filter_frame, text="رقم العميل", variable=self.search_mode, value="id").pack(side=tk.LEFT, padx=5)

        # Search input
        input_frame = tk.Frame(search_win)
        input_frame.pack(pady=10)
        tk.Label(input_frame, text="أدخل القيمة:", anchor="e", width=15).grid(row=0, column=1, padx=5)
        self.entry_search_value = tk.Entry(input_frame, justify="right", width=30)
        self.entry_search_value.grid(row=0, column=0, padx=5)
        self.entry_search_value.bind("<Return>", lambda e: self.perform_search())

        # Search button
        tk.Button(search_win, text="بحث", font=("Arial", 11), command=self.perform_search).pack(pady=5)
        ...
        self.search_result_tree = ttk.Treeview(tree_frame, columns=columns, show="headings")
        self.search_result_tree.heading("id", text="رقم")
        self.search_result_tree.heading("name", text="الاسم")
        self.search_result_tree.heading("phone", text="رقم الهاتف")
        self.search_result_tree.heading("employer", text="الموظف")
        self.search_result_tree.pack(fill=tk.BOTH, expand=True)
        self.search_result_tree.bind("<Double-1>", self.show_customer_details_from_search)
        ...
        btn_add_new_mix.config( width=20 ,bg="#4B352A" ,fg="white" )
    ...
    def perform_search(self):
        ...
        value = self.entry_search_value.get().strip()

        if not value:
            messagebox.showwarning("تنبيه", "يرجى إدخال قيمة البحث")
            return
        ...
        if field == "name":
            query = """
                SELECT id, first_name || ' ' || last_name AS full_name, phone, employer
                FROM customers
            ...
            query = """
                SELECT id, first_name || ' ' || last_name, phone, employer
                FROM customers
                WHERE phone LIKE ?
            """
            args = (f"%{value}%",)
        ...
        try:
            ...
            for row in results:
                self.search_result_tree.insert("", "end", values=row)
    ...
    def add_new_mix_record(self):
        ...
        entry_details = tk.Entry(new_win, justify="right", width=50)
        entry_details.grid(row=1, column=0, padx=5, pady=5)
